...
const DEFAULT_STEPS: StepItem[] = [
  {
    year: '2015', solutions: 45,  height: 'h-32',      color: 'from-blue-600/30',
    tech: ['Web', 'iOS'],
    logos: [
  ...
  {
    year: '2017', solutions: 80,  height: 'h-44',      color: 'from-blue-500/30',
    tech: ['AWS', 'Micro'],
    logos: [
  ...
  {
    year: '2019', solutions: 90,  height: 'h-56',      color: 'from-indigo-500/30',
    tech: ['DevOps', 'Web3'],
    logos: [
  ...
  {
    year: '2021', solutions: 110, height: 'h-64',      color: 'from-purple-500/30',
    tech: ['AR/VR', 'Alexa'],
    logos: [
  ...
  {
    year: '2023', solutions: 135, height: 'h-72',      color: 'from-pink-500/30',
    tech: ['Meta', 'NFT'],
    logos: [
  ...
  {
    year: '2025', solutions: 155, height: 'h-80',      color: 'from-emerald-500/30',
    tech: ['Gen AI', 'ML'],
    logos: [
  ...
  {
    year: '2026', solutions: 175, height: 'h-[380px]', color: 'from-cyan-400/30',
    tech: ['Quantum', 'LLM'],
    logos: [
...
const DEFAULT_STEPS_JSON = JSON.stringify(DEFAULT_STEPS);
...
const CompanyJourney = () => {
  ...
  // ── CMS values ────────────────────────────────────────────────────────────
  const eyebrow     = get('header', 'eyebrow',              'Institutional Growth Matrix');
  const headline1   = get('header', 'headline_1',           'EVOLUTION');
  const headline2   = get('header', 'headline_2',           'TIMELINE');
  const accentColor = get('header', 'accent_color',         '#22d3ee');

  const stepsRaw    = get('timeline', 'steps_json',          DEFAULT_STEPS_JSON);
  const steps       = safeParse<StepItem[]>(stepsRaw,        DEFAULT_STEPS);
  const animDurRaw  = get('timeline', 'animation_duration_ms', '2500');
  const animDur     = parseInt(animDurRaw, 10) || 2500;
  ...
  const animateNumbers = () => {
    ...
    const update = (currentTime: number) => {
      ...
      setCounts(steps.map(s => Math.floor(eased * s.solutions)));
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-20 pointer-events-none" aria-hidden="true">
        ...
        <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse at top, ${accentColor}18 0%, transparent 60%)` }} />
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <header className="text-center mb-12 md:mb-24">
          <span
            className="font-mono text-[10px] md:text-xs tracking-[0.5em] uppercase mb-4 block animate-pulse"
          ...
          <h2 className="text-5xl md:text-8xl font-black tracking-tighter mb-6 leading-none italic">
            ...
            <span className="text-transparent bg-clip-text"
              style={{ backgroundImage: 'linear-gradient(to bottom, #ffffff, rgba(255,255,255,0.2))' }}>
              {headline2}
            </span>
          </h2>
          <div
            ...
            >
              ...
              <div className="absolute -top-12 flex flex-wrap justify-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-y-2 group-hover:translate-y-0 z-30">
                {step.tech.map(t => (
                  <span
                    key={t}
                    className="bg-white/10 backdrop-blur-md px-2 py-0.5 rounded-full text-[9px] font-bold border border-white/20"
                    style={{ color: accentColor }}
                  >
                    {t}
                  </span>
              ...
              {/* Bar */}
              <div
                className={`relative w-full ${step.height} transition-all duration-[2000ms] bg-gradient-to-t ${step.color} to-transparent border-t border-x border-white/10 rounded-t-2xl group-hover:border-white/30 flex flex-col items-center justify-start pt-6 overflow-hidden shadow-[inset_0_20px_50px_rgba(0,0,0,0.5)]`}
                style={{
              ...
              >
                ...
                {/* Scanline */}
                <div
                  className="absolute top-0 left-0 w-full h-[2px] animate-scanline pointer-events-none"
              ...
              <div className="mt-6 text-center w-full bg-[#0a0c14]/50 backdrop-blur-sm p-4 rounded-b-2xl border-x border-b border-white/5 group-hover:bg-[#0a0c14] transition-colors duration-500">
                <time
                  className="text-xl md:text-3xl font-black text-gray-400 group-hover:transition-colors block mb-4 italic tracking-tighter"
                  ...
                  {step.logos.map((logo, lIdx) => (
                    ...
                    >
                      <img
                        ...
                        onError={e => {
                          (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${logo.name}&background=1e293b&color=22d3ee&bold=true`;
                        }}
                      />
                    </div>
                  ))}
