  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      for (let i = 0; i < numStars; i++) {
        stars.push({ x: Math.random() * canvas.width - canvas.width / 2, y: Math.random() * canvas.height - canvas.height / 2, z: Math.random() * canvas.width, o: Math.random() });
      }
    };
...
export default function HeroB2B() {
  ...
  // CMS values
  const badgeLabel      = get('hero', 'badge_label',      'The Future of Work is Autonomous');
  const headlineLine1   = get('hero', 'headline_line1',   'Command Your');
  const headlineAccent  = get('hero', 'headline_accent',  'AI Empire');
  const bodyText        = get('hero', 'body_text',        'Deploy custom AI workforces that execute tasks autonomously. Reduce overhead by 70% and scale instantly.');
  const bodyHighlight   = get('hero', 'body_highlight',   'execute tasks');
  const btnPrimaryLabel = get('hero', 'btn_primary_label','Deploy AI Workforce');
  const whatsappNumber  = get('hero', 'whatsapp_number',  '918700236923');
  const whatsappMessage = get('hero', 'whatsapp_message', 'I want to deploy AI Agents.');
  const btnDemoLabel    = get('hero', 'btn_demo_label',   'Watch Demo');
  ...
  const numStarsRaw     = get('hero', 'num_stars',        '400');
  const numStars        = parseInt(numStarsRaw, 10) || 400;

  const avatarsRaw = get('hero', 'avatars_json', '');
  const avatars    = useMemo(() => safeParse<{ src: string; name: string }[]>(avatarsRaw, DEFAULT_AVATARS), [avatarsRaw]);

  // Highlight bodyText
  const renderBody = () => {
    if (!bodyHighlight || !bodyText.includes(bodyHighlight)) {
      return <>{bodyText}</>;
    }
    const [before, after] = bodyText.split(bodyHighlight);
    return (
      <>{before}<span className="text-white font-bold underline decoration-blue-500/50">{bodyHighlight}</span>{after}</>
    );
  };
      ...
      >
        <div className="flex flex-col items-center">
          ...
          >
            <div className="flex -space-x-4">
              {avatars.map((avatar, i) => (
                <div key={i} className="relative w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-[#020617] overflow-hidden bg-slate-800">
                  <Image src={avatar.src} alt={avatar.name} fill sizes="48px" className="object-cover" priority={i < 2} />
                </div>
              ))}
            </div>
            ...
            <p className="text-blue-400 text-[10px] md:text-[9px] font-black tracking-[0.15em] uppercase flex items-center gap-2">
              ...
              <span style={{ color: accentColor }}>{badgeLabel}</span>
            </p>
          </motion.div>

          <h1 className="text-5xl sm:text-5xl md:text-7xl font-black text-white mb-8 tracking-tighter leading-[0.85] uppercase">
            {headlineLine1} <br />
            <span className="relative inline-block italic" style={{ color: accentColor }}>
              {headlineAccent}
              <svg className="absolute -bottom-2 md:-bottom-4 left-0 w-full" viewBox="0 0 300 12" fill="none">
                <path d="M1 9.5C50.5 3.5 150.5 1.5 299 9.5" stroke={accentColor} strokeWidth="4" strokeLinecap="round" />
              </svg>
            </span>
          </h1>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-5 justify-center items-center">
          <button
            ...
            style={{ backgroundColor: accentColor, boxShadow: `0 0 40px -10px ${accentColor}99` }}
          >
            {btnPrimaryLabel} <ArrowRight size={18} />
          </button>
          ...
          >
            <Play className="w-4 h-4 fill-white" /> {btnDemoLabel}
          </button>
        </div>
      </motion.div>
    </section>
