...
const DynamicCounter: React.FC<CounterProps> = ({ initialValue, min, max, intervalMs = 3000, suffix = "" }) => {
  ...
  useEffect(() => {
  let direction = Math.random() > 0.3 ? 1 : -1;
  ...
  const updateNumber = () => {
    setCount((prev) => {
      ...
      // 1. Occasional Big Updates (5% chance)
      // Moves roughly 5-10% of the total range in one go
      if (chance > 0.65) {
        change = Math.floor(Math.random() * (range * 0.05)) + Math.floor(range * 0.05);
      } 
      // 2. Normal Smooth Updates
      else {
        change = Math.floor(Math.random() * (range * 0.03)) + 1;
      }

      // 3. Gravity/Bias Logic
      // Higher numbers are more likely to head down, lower head up
      // This ensures it eventually touches both MIN and MAX
      if (Math.random() < proximityToMax) {
        direction = -1;
      } else {
        direction = 1;
      }
      ...
      // 4. Boundary Protection & Clipping
    ...
    // Variable timing to make it feel less robotic
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <StatCard 
            ...
            value={<DynamicCounter initialValue={8420} min={8000} max={9900} suffix="+" intervalMs={1000} />}
          ...
          <StatCard 
            ...
            value={<DynamicCounter initialValue={3150} min={3000} max={4200} intervalMs={1800} />}
          ...
          <StatCard 
            title="API Requests"
            value={<DynamicCounter initialValue={142} min={120} max={180} suffix="/s" intervalMs={1200} />}
          ...
          <StatCard 
            ...
            value={<DynamicCounter initialValue={512} min={510} max={525} intervalMs={2000} />}
