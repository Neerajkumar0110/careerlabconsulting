...
const DynamicCounter: React.FC<CounterProps> = ({
  initialValue,
  min,
  max,
  intervalMs = 3000,
...
const StatCard: React.FC<StatCardProps> = ({ data, delay, liveBadgeLabel, accentColor }) => {
  ...
  return (
    ...
    >
      {/* Glow blob */}
      <div
        className={`absolute -right-4 -top-4 w-24 h-24 blur-3xl opacity-10 transition-opacity group-hover:opacity-30 ${data.colorClass}`}
      />
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <div
          ...
          style={{ background: '#10b98133', borderColor: '#10b98133' }}
        >
      ...
      <div className="relative z-10">
        <p className="text-slate-400 text-xs font-semibold uppercase tracking-[0.15em] mb-1">
          {data.title}
        </p>
...
const LiveTrafficSection: React.FC = () => {
  const { get } = usePageContent('live-traffic');
