...
// ── Animated counter ──────────────────────────────────────────────────────────
interface CounterProps { value: number; inView: boolean; }
...
// ── Types ─────────────────────────────────────────────────────────────────────
interface MilestoneLogo { url: string; name: string; }
interface MilestoneStep {
  year: string; label: string; solutions: number;
  tech: string[]; desc: string; color: string; glow: string;
  logos: MilestoneLogo[];
}
...
const DEFAULT_STEPS_JSON = JSON.stringify([
  {
    year: "2015", solutions: 45, label: "Foundation",
  ...
  {
    year: "2017", solutions: 80, label: "Expansion",
  ...
  {
    year: "2019", solutions: 90, label: "Enterprise Grade",
  ...
  {
    year: "2021", solutions: 110, label: "Immersive Era",
  ...
  {
    year: "2023", solutions: 135, label: "Web3 & Meta",
  ...
  {
    year: "2025", solutions: 155, label: "AI-Native",
  ...
  {
    year: "2026", solutions: 175, label: "Quantum Leap",
...
// ── Milestone Card ────────────────────────────────────────────────────────────
function MilestoneCard({ step, index }: { step: MilestoneStep; index: number }) {
  ...
  return (
    <motion.div ref={ref}
      ...
      <div className="relative rounded-[28px] overflow-hidden transition-all duration-500"
        style={{ background: 'linear-gradient(145deg, rgba(15,23,42,0.95) 0%, rgba(8,12,28,0.98) 100%)', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 24px 64px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.04)' }}>
        ...
        <div className="p-7 md:p-8">
          <div className="flex flex-col sm:flex-row items-start justify-between mb-4 sm:mb-5 gap-4">
            <div>
              <div className="flex flex-wrap items-baseline gap-2 sm:gap-3 mb-1">
                <span className="font-black leading-none"
                  style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(1.8rem, 8vw, 3.5rem)', background: `linear-gradient(135deg, ${step.color} 0%, rgba(255,255,255,0.6) 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', letterSpacing: '-0.04em' }}>
                  {step.year}
                </span>
                <span className="text-[10px] sm:text-xs font-semibold uppercase tracking-[0.2em] px-2.5 py-1 rounded-full"
                  style={{ background: `${step.color}18`, color: step.color, border: `1px solid ${step.color}30`, fontFamily: "'DM Mono', monospace" }}>
                  {step.label}
                </span>
              </div>
            ...
            <div className="flex flex-wrap sm:flex-col gap-1.5 items-start sm:items-end">
              {step.tech.map((t) => (
                <span key={t} className="text-[8px] sm:text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-lg"
                  style={{ fontFamily: "'DM Mono', monospace", background: `${step.color}12`, color: `${step.color}cc`, border: `1px solid ${step.color}25` }}>
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
            {step.logos.map((logo, lIdx) => (
              <motion.div key={lIdx}
                ...
                style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(255,255,255,0.05)', transition: 'all 0.3s ease' }}
                whileHover={{ scale: 1.08, y: -2 }}>
                <div className="absolute inset-0 opacity-0 group-hover/logo:opacity-100 transition-opacity duration-300 rounded-xl"
                  style={{ background: `radial-gradient(ellipse at center, ${step.color}15, transparent 70%)` }} />
                <img src={logo.url} alt={logo.name}
                  className="max-h-full max-w-full object-contain relative z-10"
                  style={{ filter: 'opacity(0.95)', transition: 'filter 0.4s ease' }}
                  onMouseEnter={e => (e.currentTarget.style.filter = 'drop-shadow(0 0 8px #ffffff)')}
                  onMouseLeave={e => (e.currentTarget.style.filter = 'opacity(0.95)')} />
              </motion.div>
            ))}
          </div>
        </div>
...
const CompanyJourney = () => {
  const { get } = usePageContent('company-journey');
  const containerRef = useRef(null);
  const { scrollYProgress } = useScroll({ target: containerRef, offset: ['start end', 'end end'] });
  const pathLength   = useSpring(scrollYProgress, { stiffness: 300, damping: 80 });

  // CMS values
  const badgeText        = get('header', 'badge_text',      'Institutional Growth Matrix');
  const headline1        = get('header', 'headline_1',      'EVOLUTION');
  const headline2        = get('header', 'headline_2',      'TIMELINE');
  const subtext          = get('header', 'subtext',         'A decade of partnerships, products, and pioneering technology across every industry.');
  const endCapText       = get('header', 'end_cap_text',    'The Journey Continues');

  const stepsRaw = get('steps', 'steps_json', DEFAULT_STEPS_JSON);
  const steps    = safeParse<MilestoneStep[]>(stepsRaw, []);
  ...
  return (
    <section ref={containerRef}
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.015]"
        style={{ backgroundImage: 'repeating-linear-gradient(45deg, rgba(255,255,255,0.5) 0px, rgba(255,255,255,0.5) 1px, transparent 1px, transparent 12px)' }} />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-32 relative">
          <motion.div initial={{ opacity: 0, y: -10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            ...
            <span className="text-[10px] text-indigo-300 tracking-[0.3em] uppercase">{badgeText}</span>
          </motion.div>

          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1, duration: 0.8 }}
            className="font-black uppercase leading-[0.9] tracking-tighter mb-6"
            style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(3.5rem, 10vw, 7rem)' }}>
            <span className="text-white">{headline1}</span><br />
            <span style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.9) 0%, rgba(34,211,238,0.7) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
              {headline2}
            </span>
          </motion.h2>

          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.25 }}
            className="text-sm text-slate-500 max-w-sm mx-auto" style={{ letterSpacing: '0.07em' }}>
            {subtext}
          </motion.p>
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <svg className="absolute left-1/2 -translate-x-1/2 w-full pointer-events-none hidden md:block"
            ...
            <path
              d={`M200 0 ${steps.map((_, i) => {
                const y = i * 370 + 185;
                return i % 2 === 0
                  ? `C 450 ${y - 120}, -50 ${y + 120}, 200 ${y + 185}`
                  : `C -50 ${y - 120}, 450 ${y + 120}, 200 ${y + 185}`;
              }).join(' ')}`}
            ...
            <motion.path
              ...
              stroke="url(#spine-grad)" strokeWidth="2" strokeDasharray="12 8"
                ...
                <div key={step.year}
                  ...
                  <div className="absolute left-5 md:left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2 z-30 flex flex-col items-center">
                    <motion.div whileHover={{ scale: 1.6 }}
                      ...
                      viewport={{ once: true }} transition={{ type: 'spring', stiffness: 400, damping: 20 }}
                      ...
                      <div className="relative w-5 h-5 rounded-full border-2"
                        style={{ backgroundColor: step.color, borderColor: '#020617', boxShadow: `0 0 24px ${step.glow}, 0 0 8px ${step.color}` }} />
                    </motion.div>
                    <motion.span initial={{ opacity: 0, y: 6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
            ...
            <div className="flex items-center gap-3 px-6 py-3 rounded-full"
              style={{ background: 'rgba(34,211,238,0.07)', border: '1px solid rgba(34,211,238,0.25)', boxShadow: '0 0 32px rgba(34,211,238,0.1)' }}>
