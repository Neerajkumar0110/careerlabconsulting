...
import { 
  ArrowUpRight, Home, Briefcase, Phone, X, Globe, MapPin, 
  Navigation, Facebook, Twitter, Instagram, Linkedin, Youtube, 
  Search, User, Mail 
} from 'lucide-react';
...
export default function Footer() {
  ...
  const whatsappNumber = "918700827753";
  ...
  return (
    <>
      <footer className="bg-[#020617] pt-20 pb-24 md:pb-12 border-t border-white/5 relative overflow-hidden" aria-labelledby="footer-heading">
        <h2 id="footer-heading" className="sr-only">Career Lab Consulting - Global Talent Platform</h2>
        ...
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
            ...
            <div className="lg:col-span-3 flex flex-col items-start">
              ...
              <p className="text-slate-400 text-sm leading-relaxed font-medium mb-10">
                Global talent marketplace powered by autonomous verification nodes. Architecting the next generation of digital workforces through true cognitive independence.
              </p>
              
              <div className="flex flex-wrap gap-3">
                {SOCIAL_LINKS.map(({ Icon, href, color, name }, i) => (
                  <a 
                    key={i} 
                    href={href} 
                    target="_blank"
            ...
            <div className="flex items-center justify-between mb-10 border-b border-white/10 pb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mt-1">Operating across primary tech hubs</p>
                </div>
              </div>
            </div>
              ...
              {LOCATIONS.map((loc, idx) => (
                <div key={idx} className="group relative bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-blue-500/40 rounded-3xl p-7 transition-all duration-500 flex flex-col h-full hover:-translate-y-2">
                  ...
                  <div className="space-y-4">
                    ...
                    <a 
                      href={`http://maps.google.com/?q=${encodeURIComponent(loc.address)}`} 
                      target="_blank" 
                      rel="noopener noreferrer" 
          ...
          <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex flex-wrap justify-center items-center gap-x-8 gap-y-4">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">© 2015 - {currentYear} FreelanceX</span>
            ...
            <div className="flex items-center gap-3 px-5 py-2.5 bg-emerald-500/5 border border-emerald-500/10 rounded-full">
              ...
              <span className="text-[10px] text-emerald-500/90 font-black uppercase tracking-widest">Protocol Active: X-Secure</span>
            </div>
          </div>
        </div>
      </footer>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md h-20 bg-[#0a0f1d]/90 backdrop-blur-xl border border-white/10 rounded-[2.5rem] z-[100] md:hidden shadow-2xl flex items-center justify-around px-2">
        ...
        <Link 
          ...
          className="relative -top-7 w-16 h-16 bg-green-500 rounded-full border-[6px] border-[#020617] flex items-center justify-center shadow-[0_0_20px_rgba(34,197,94,0.4)] transition-transform active:scale-90"
        >
        ...
        <Link href="#test" className="flex flex-col items-center justify-center min-w-[64px] h-full text-slate-500 hover:text-blue-400 transition-colors">
          <Search className="w-6 h-6 mb-1" aria-hidden="true" />
          <span className="text-[9px] font-black uppercase tracking-tighter">Tests</span>
        </Link>
        
        <Link href="#" className="flex flex-col items-center justify-center min-w-[64px] h-full text-slate-500 hover:text-blue-400 transition-colors">
          <User className="w-6 h-6 mb-1" aria-hidden="true" />
