...
const DEFAULT_PILLARS: PillarItem[] = [
  {
    ...
    cta_url:  '/contact',
    stats:    [{ label: 'Students', value: '15000k+' }, { label: 'Trainers', value: '1.5k+' }, { label: 'Partners', value: '350+' }],
  },
  {
    ...
    cta_url:  '/contact',
    stats:    [{ label: 'Users', value: '50k+' }, { label: 'Courses', value: '120+' }, { label: 'Uptime', value: '99.9%' }],
  },
  {
    ...
    cta_url:  '/contact',
    stats:    [{ label: 'Companies', value: '200+' }, { label: 'Executives', value: '5k+' }, { label: 'Growth', value: '10X' }],
  },
  {
    ...
    cta_url:  '/contact',
    stats:    [{ label: 'Projects', value: '85+' }, { label: 'Patents', value: '12' }, { label: 'Experts', value: '45+' }],
  },
  {
    ...
    cta_url:  '/contact',
    stats:    [{ label: 'Placed', value: '12k+' }, { label: 'Avg Hike', value: '60%' }, { label: 'Clients', value: '400+' }],
  },
  {
    ...
    image:    'https://plus.unsplash.com/premium_photo-1664474503673-5febe9ca12b9?w=600&auto=format&fit=crop&q=60',
    accent:   'from-indigo-600 to-violet-500',
    cta_url:  '/contact',
    stats:    [{ label: 'Countries', value: '27+' }, { label: 'Alumni', value: '100k' }, { label: 'Badges', value: '50+' }],
  },
];
const DEFAULT_PILLARS_JSON = JSON.stringify(DEFAULT_PILLARS);

// ─────────────────────────────────────────────────────────────────────────────

export default function PillarsSection() {
  const { get } = usePageContent('pillars-section');

  // ── CMS values ────────────────────────────────────────────────────────────
  const eyebrow        = get('header', 'eyebrow',         'Why Choose Us');
  const headlinePlain  = get('header', 'headline_plain',  'Our Six Pillars of');
  const headlineAccent = get('header', 'headline_accent', 'Excellence');
  const ctaLabel       = get('header', 'cta_label',       'Know More');

  const pillarsRaw = get('pillars', 'items_json', DEFAULT_PILLARS_JSON);
    ...
    <section className="py-16 md:py-32 bg-[#020617] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 md:px-6 relative z-10">
        ...
        <div className="text-center mb-16 md:mb-32">
          <span className="text-blue-500 font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase text-[9px] md:text-[10px] block mb-3">
            {eyebrow}
          </span>
          <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-white uppercase tracking-tighter leading-tight">
            {headlinePlain}{' '}
            <br className="block md:hidden" />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-emerald-400 italic">
              {headlineAccent}
            </span>
          </h2>
        </div>
          ...
          {pillars.map((pillar, idx) => (
            ...
            >
              ...
              <div className="w-full lg:w-1/2 space-y-6 md:space-y-8 text-center lg:text-left">
                ...
                {/* CTA */}
                <div className="pt-4">
                  <Link href={pillar.cta_url || '/contact'} className="w-full md:w-auto">
                    <button
                      className={`group w-full md:w-auto flex items-center justify-center gap-3 px-8 py-4 rounded-xl md:rounded-2xl bg-gradient-to-r ${pillar.accent} text-white font-black text-[10px] md:text-xs uppercase tracking-widest hover:brightness-110 transition-all active:scale-95 shadow-lg shadow-blue-900/20`}
                    >
                      {ctaLabel}
