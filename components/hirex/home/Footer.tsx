...
import { 
  ...
  Navigation, Facebook, Instagram, Youtube, ShieldCheck, 
  Activity, Cpu, Zap
} from 'lucide-react';
...
const socialLinks = [
  ...
  { Icon: Linkedin, href: "https://www.linkedin.com/company/careerlabconsulting", color: "hover:bg-[#0A66C2]", name: "Linkedin" },
...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#020617] pt-20 pb-24 md:pb-12 border-t border-white/5 relative overflow-hidden font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
          ...
          <div className="lg:col-span-3 flex flex-col items-start">
            <Link href="/hirex" className='pb-5'>
              <img src="/logo.png" alt="Career Lab Consulting" className="object-contain h-10 w-auto" />
            </Link>
            ...
            <p className="text-slate-400 text-sm leading-relaxed font-medium mb-10">
              The world's first autonomous verification ecosystem. Empowering global enterprises through high-fidelity cognitive intelligence.
            </p>
          ...
          {/* Links Grid */}
          <div className="lg:col-span-5 grid grid-cols-2 md:grid-cols-2 gap-8 pt-4">
                  ...
                  {links.map((link) => (
                    <li key={link.name}>
                      <Link href={link.href} className="hover:text-blue-400 transition-colors flex items-center gap-1 group">
                        {link.name} 
                        <ArrowUpRight className="w-3 h-3 text-blue-500/50 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-4">
            <div className="p-6 rounded-3xl bg-gradient-to-br from-blue-600/10 to-transparent border border-blue-500/20 backdrop-blur-md">
              ...
              <address className="not-italic space-y-5">
                <a href="https://maps.app.goo.gl/YourLink" target="_blank" className="flex items-start gap-4 group">
                  <div className="w-10 h-10 rounded-xl bg-blue-600/20 flex items-center justify-center shrink-0">
                    <MapPin className="w-5 h-5 text-blue-400" />
                  </div>
                  <p className="text-slate-300 text-sm leading-relaxed font-bold group-hover:text-blue-400 transition-colors">
                    DLF Cyber City, 5th Floor, Cyber Green-1, Sec-25, Gurugram, India
                  </p>
                </a>
        ...
        <div className="pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-wrap justify-center items-center gap-x-8 gap-y-4">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">© 2015 - {currentYear} CLC HIREX</span>
            <div className="flex items-center gap-2 text-blue-500/80 text-[10px] font-bold uppercase tracking-widest">
              <ShieldCheck size={12} />
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md h-20 bg-[#0a0f1d]/90 backdrop-blur-xl border border-white/10 rounded-[2.5rem] z-[100] md:hidden shadow-2xl flex items-center justify-around px-2">
        <Link href="/hirex" className="flex flex-col items-center justify-center min-w-[64px] h-full text-slate-500 hover:text-blue-400 transition-colors">
        ...
        <Link href="/hirex/job-inventory" className="flex flex-col items-center justify-center min-w-[64px] h-full text-slate-500 hover:text-blue-400 transition-colors">
          <Briefcase className="w-6 h-6 mb-1" />
          <span className="text-[9px] font-black uppercase tracking-tighter">Jobs</span>
        </Link>
        <Link href="/hirex/support" className="flex flex-col items-center justify-center min-w-[64px] h-full text-slate-500 hover:text-blue-400 transition-colors">
          <Phone className="w-6 h-6 mb-1" />
          <span className="text-[9px] font-black uppercase tracking-tighter">Support</span>
        </Link>
      </nav>

      {/* Mobile Enterprise Modal */}
      {isProductOpen && (
        <div className="md:hidden fixed inset-0 z-[110] flex items-end justify-center px-4 pb-24 bg-black/80 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-[#0a0f1d] border border-white/10 rounded-[2.5rem] p-6 shadow-2xl overflow-hidden max-h-[70vh]">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-white font-black text-xs uppercase tracking-[0.2em]">Enterprise Core</h3>
              <button onClick={() => setIsProductOpen(false)} className="p-2 bg-white/5 rounded-full text-slate-400"><X className="w-6 h-6" /></button>
            </div>
            <div className="grid grid-cols-1 gap-1 overflow-y-auto max-h-[50vh] pr-2">
              {productList.map((item) => (
                <button key={item} onClick={() => { setIsProductOpen(false); window.open(`https://wa.me/${whatsappNumber}`, '_blank'); }} 
                        className="text-left text-slate-400 hover:text-blue-400 text-sm font-bold py-4 border-b border-white/5 transition-colors uppercase tracking-wider">
                  {item}
                </button>
              ))}
            </div>
