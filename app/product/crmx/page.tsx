      ...
      <section className="relative min-h-screen flex flex-col justify-center overflow-hidden pt-20 pb-12 px-5 sm:px-8 lg:px-12">
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute inset-0 opacity-[0.025]"
            style={{ backgroundImage: `radial-gradient(${gradFrom}FF 1px, transparent 1px)`, backgroundSize: '36px 36px' }} />
        </div>
                  ...
                  {CHAT_MESSAGES.slice(0, visibleMessages).map((msg, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl ${msg.side === 'out' ? 'rounded-br-sm text-white' : 'bg-white/[0.05] border border-white/10 text-slate-300 rounded-bl-sm'}`}
                        style={msg.side === 'out' ? { background: `linear-gradient(to right, ${gradFrom}, ${gradTo})` } : {}}>
                        <p className="text-xs leading-relaxed">{msg.text}</p>
                        <p className={`text-[9px] mt-1 ${msg.side === 'out' ? 'text-white/50' : 'text-slate-600'}`}>{msg.time}</p>
                      </div>
                    </motion.div>
                  ))}
            ...
            <div>
              ...
              <div className="mt-8">
                <WhatsAppButton
                  ...
                  shadowColor="shadow-[0_0_40px_rgba(37,99,235,0.3)]"
                  hoverShadowColor="hover:shadow-[0_0_60px_rgba(37,99,235,0.5)]"
                />
              </div>
            </div>
