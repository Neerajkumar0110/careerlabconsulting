// app/internx/ai-iot-engineer/page.tsx
...
import {
  Wifi, Cpu, Brain, Cloud, Activity, Settings,
  ArrowRight, X, Loader2, ChevronRight, Mail, Users,
} from 'lucide-react';
...
const ICON_MAP: Record<string, React.ElementType> = {
  Wifi, Cpu, Brain, Cloud, Activity, Settings, ArrowRight,
};
...
const DEFAULT_STATS = JSON.stringify([
  ...
  { label: 'Smart Projects', value: '9+'  },
  { label: 'IoT Protocols',  value: '6+'  },
  { label: 'Career Support', value: '100%'},
]);
const DEFAULT_CHIPS = JSON.stringify([
  { icon: 'Wifi',     label: 'IoT Systems'          },
  { icon: 'Brain',    label: 'AI at the Edge'       },
  { icon: 'Cloud',    label: 'Cloud IoT Platforms'  },
  { icon: 'Activity', label: 'Real-Time Analytics'  },
]);
const DEFAULT_WHY_FEATURES = JSON.stringify([
  { icon: 'Wifi',     title: 'Connected Intelligence', desc: 'IoT devices generate massive data that AI turns into real-time insights.' },
  { icon: 'Cpu',      title: 'Edge Computing',         desc: 'Run AI models directly on devices for low-latency decisions.' },
  { icon: 'Settings', title: 'Smart Automation',       desc: 'Automate environments using intelligent IoT systems.' },
]);
...
const DEFAULT_EXPERIENCE_FEATURES = JSON.stringify([
  { icon: 'Brain',     title: 'AI + IoT Projects',  desc: 'Hands-on work with connected devices and AI-driven insights.' },
  { icon: 'Cloud',     title: 'Cloud Integration',  desc: 'Connect devices to scalable cloud platforms.' },
  { icon: 'ArrowRight',title: 'Career Readiness',   desc: 'Portfolio projects, interview prep, and IoT career guidance.' },
]);
...
export default function InternXAIoTEngineerPage() {
  ...
  const { get } = usePageContent('internx-aiot');

  // ── CMS values ────────────────────────────────────────────────────────────
  const accentColor      = get('hero', 'accent_color',     '#22c55e');
  const accentShadow     = get('hero', 'accent_shadow',    'rgba(34,197,94,0.3)');
  const badgeText        = get('hero', 'badge_text',       'InternX Program');
  const heroHeadline     = get('hero', 'headline',         'AI & IoT Engineer Internship');
  const heroBody         = get('hero', 'body_text',        'Design intelligent, connected systems powered by AI. Work on smart devices, sensors, edge intelligence, and cloud-based IoT platforms used in smart homes, cities, and industries.');
  const heroImageUrl     = get('hero', 'hero_image_url',   'https://img.freepik.com/free-photo/teens-doing-experiments-robotics-laboratory-boy-protective-glasses-touching-robot_1268-23410.jpg');
  ...
  const whySectionTitle  = get('why', 'section_title', 'Why AI + IoT?');
  ...
  const ctaHeadline      = get('cta', 'headline',          'Build Intelligent Connected Systems');
  const ctaBody          = get('cta', 'body_text',         'Launch a future-ready career at the intersection of AI and IoT.');
  const ctaApplyLabel    = get('cta', 'apply_btn_label',   'Apply Now');
  const ctaAdvisorLabel  = get('cta', 'advisor_btn_label', 'Talk to Advisor');

  const modalBadge       = get('apply_modal', 'badge_label',      'AI & IoT Internship – Apply');
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    const msg = `*📡 AI & IoT Engineer Internship Application*%0A*Name:* ${formData.name}%0A*Email:* ${formData.email}`;
    setTimeout(() => {
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {chipItems.map((chip, i) => {
                const Icon = ICON_MAP[chip.icon] ?? Wifi;
                return (
