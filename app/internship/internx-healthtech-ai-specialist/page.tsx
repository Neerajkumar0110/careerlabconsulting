// app/internx/healthtech-ai-specialist/page.tsx
...
import {
  HeartPulse, Brain, BarChart3, ShieldCheck,
  Stethoscope, ArrowRight, Activity, X, Loader2,
...
const ICON_MAP: Record<string, React.ElementType> = {
  HeartPulse, Brain, BarChart3, ShieldCheck, Stethoscope, ArrowRight, Activity,
};
...
const DEFAULT_STATS = JSON.stringify([
  ...
  { label: 'Healthcare Use Cases', value: '7+' },
  { label: 'AI Models', value: '10+' },
  { label: 'Career Support', value: '100%' },
]);
const DEFAULT_WHY_FEATURES = JSON.stringify([
  { icon: 'Stethoscope', title: 'Healthcare Innovation', desc: 'AI is transforming diagnostics, treatment planning, and patient care.' },
  { icon: 'ShieldCheck',  title: 'Safety & Compliance',  desc: 'Build systems aligned with medical regulations and data privacy standards.' },
  { icon: 'HeartPulse',  title: 'Human Impact',          desc: 'Create AI solutions that directly improve patient outcomes and lives.' },
]);
...
const DEFAULT_EXPERIENCE_FEATURES = JSON.stringify([
  { icon: 'Brain',     title: 'Health AI Projects', desc: 'Work on real healthcare datasets and AI-driven medical use cases.' },
  { icon: 'BarChart3', title: 'Clinical Insights',  desc: 'Extract insights from complex medical and patient data.' },
  { icon: 'ArrowRight',title: 'Career Readiness',   desc: 'Portfolio projects, interview prep, and HealthTech career guidance.' },
]);
...
export default function InternXHealthTechAISpecialistPage() {
  ...
  const { get } = usePageContent('internx-healthtech');

  // ── CMS values ────────────────────────────────────────────────────────────
  const accentColor      = get('hero', 'accent_color',     '#ef4444');
  const accentShadow     = get('hero', 'accent_shadow',    'rgba(239,68,68,0.3)');
  const badgeText        = get('hero', 'badge_text',       'InternX Program');
  const heroHeadline     = get('hero', 'headline',         'HealthTech AI Specialist Internship');
  const heroBody         = get('hero', 'body_text',        'Build intelligent healthcare solutions using AI. Work on real-world use cases like medical data analysis, predictive diagnostics, patient monitoring, and AI-assisted clinical decision systems.');
  const heroImageUrl     = get('hero', 'hero_image_url',   'https://img.freepik.com/free-photo/scientific-specialist-research-lab-wearing-vr-goggles-using-high-tech-equipment-wired-sensors-medical-study-healthcare-practitioner-using-virtual-reality-technology-visualize-datasets_482257-67903.jpg');
  ...
  const whySectionTitle  = get('why', 'section_title', 'Why HealthTech + AI?');
  ...
  const ctaHeadline      = get('cta', 'headline',          'Build AI Solutions That Save Lives');
  const ctaBody          = get('cta', 'body_text',         'Launch a meaningful career at the intersection of AI and healthcare.');
  const ctaApplyLabel    = get('cta', 'apply_btn_label',   'Apply Now');
  const ctaAdvisorLabel  = get('cta', 'advisor_btn_label', 'Talk to Advisor');

  const modalBadge       = get('apply_modal', 'badge_label',   'HealthTech AI Internship – Apply');
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    const msg = `*🏥 HealthTech AI Internship Application*%0A*Name:* ${formData.name}%0A*Email:* ${formData.email}`;
    setTimeout(() => {
