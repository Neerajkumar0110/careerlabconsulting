...
import { 
  ...
  Code2, MessageCircle, Heart, Repeat
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto relative z-10 px-4 sm:px-6 lg:px-8 space-y-24">
          ...
          <div className="text-center space-y-6 md:space-y-8 max-w-4xl mx-auto">
            ...
            <h1 className="text-4xl md:text-8xl font-black tracking-tighter leading-[1.1] md:leading-[0.9]">
              The Protocol <br className="hidden sm:block"/> <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-white to-cyan-400">Community.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-3 md:gap-4 pt-4">
              <div className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-3 shadow-xl">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-300">12.4k Active Nodes</span>
              </div>
              <div className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-3 shadow-xl">
                <Globe size={14} className="text-blue-400" />
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-300">150+ Countries</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-6 md:gap-8">
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
              className="lg:col-span-7 bg-[#0a0f1d]/60 border border-white/10 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-12 backdrop-blur-2xl shadow-3xl"
            >
              <div className="flex items-center justify-between mb-8 md:mb-10">
                ...
                <span className="hidden sm:block text-[10px] font-black uppercase text-slate-500 tracking-widest">Update: Hourly</span>
              </div>
              
              <div className="space-y-4">
                {LEADERBOARD.map((user, idx) => (
                  <div key={idx} className="group bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 hover:border-indigo-500/30 p-5 md:p-6 rounded-[2rem] transition-all flex items-center justify-between">
                    <div className="flex items-center gap-4 md:gap-6">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-bold text-white group-hover:text-blue-400 transition-colors truncate">{user.name}</h4>
              ...
              <button className="w-full mt-8 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white/10 transition-all active:scale-[0.98]">View Full Ranking</button>
            </motion.div>
            ...
            >
              <div className="bg-gradient-to-br from-blue-600/20 to-indigo-600/20 border border-blue-500/20 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-10 relative overflow-hidden flex-grow flex flex-col justify-center backdrop-blur-md">
                 <div className="absolute top-[-10%] right-[-10%] opacity-10 pointer-events-none"><Gift size={200} /></div>
                 <h2 className="text-2xl md:text-3xl font-black mb-4 leading-tight">Refer & Earn <br/> <span className="text-blue-400">Exclusive Rewards.</span></h2>
                 <p className="text-slate-400 text-sm mb-8 font-medium leading-relaxed">Grow the ecosystem. Invite elite talent and receive up to 5% commission on their first 12 months of sprints.</p>
                 <div className="space-y-4">
                    <div className="bg-[#020617]/80 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
                       <code className="text-[10px] md:text-xs text-blue-400 truncate mr-4">HIREX-00X2-PROTOCOL</code>
                    ...
                    <button className="w-full py-4 md:py-5 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-xl shadow-blue-900/20 active:scale-[0.98]">Initialize Referral</button>
                 </div>
              </div>

              {/* Support Shortcut */}
              <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 flex items-center justify-between group cursor-pointer hover:bg-emerald-500/10 transition-all">
                 <div className="flex items-center gap-4">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-emerald-500/20 flex items-center justify-center">
          ...
          {/* Section 3: Knowledge Exchange (NEW) */}
          <div className="space-y-12">
             <div className="text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                   <h2 className="text-3xl md:text-5xl font-black tracking-tight mb-4">Knowledge <span className="text-cyan-400 italic">Exchange.</span></h2>
                   <p className="text-slate-500 font-medium">Decentralized engineering discussions and problem-solving.</p>
                </div>
                <button className="text-blue-400 text-[10px] font-black uppercase tracking-widest hover:text-white transition-all flex items-center justify-center gap-2 mx-auto md:mx-0">
                   Explore Forum <ArrowUpRight size={14} />
                </button>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {DISCUSSIONS.map((topic, i) => (
                   <motion.div 
                    key={i}
                    whileHover={{ y: -5 }}
                    className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-8 flex flex-col h-full relative group cursor-pointer hover:border-cyan-500/30 transition-all"
                   >
                      <div className="flex items-center gap-2 mb-6">
                        <MessageCircle size={16} className="text-cyan-400" />
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Technical Thread</span>
                      </div>
                      <h3 className="text-lg font-bold text-white mb-6 flex-grow leading-snug group-hover:text-cyan-400 transition-colors">{topic.title}</h3>
          ...
          <div className="space-y-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 text-center md:text-left">
              <div>
                 ...
                 <p className="text-slate-500 font-medium text-sm md:text-base">Reserved sessions for verified professionals.</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {EVENTS.map((event, i) => (
                <motion.div 
                  ...
                  className="bg-[#0a0f1d] border border-white/5 hover:border-indigo-500/30 rounded-[2.5rem] p-8 transition-all relative overflow-hidden group shadow-lg"
                >
                  <div className="absolute top-0 right-0 p-6 text-white/5 group-hover:text-indigo-500/10 transition-colors pointer-events-none">
                     <event.icon size={80} />
                  </div>
                  <div className="relative z-10 space-y-6">
                    <span className="inline-block px-3 py-1 rounded-lg bg-indigo-500/10 text-indigo-400 text-[9px] font-black uppercase tracking-widest border border-indigo-500/20">
                      {event.type}
                    </span>
          ...
          {/* Section 5: Final CTA */}
          <div className="relative rounded-[2.5rem] md:rounded-[3rem] overflow-hidden bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 p-[1px]">
            <div className="relative bg-[#020617] rounded-[2.45rem] md:rounded-[2.95rem] p-8 md:p-24 text-center space-y-8 md:space-y-10 overflow-hidden">
               ...
               <h2 className="text-3xl md:text-6xl font-black text-white leading-tight">Ready to integrate <br className="hidden md:block"/> with the elite?</h2>
               <p className="text-slate-400 text-sm md:text-lg max-w-xl mx-auto px-4 leading-relaxed">Join thousands of architects and engineers shaping the future of decentralized work.</p>
               <div className="flex flex-col sm:flex-row items-center justify-center gap-4 md:gap-6 px-4">
                  <Link href="/freelancex/signup" className="w-full sm:w-auto px-10 md:px-12 py-4 md:py-5 bg-white text-black font-black uppercase tracking-widest rounded-2xl hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,255,255,0.15)] text-xs md:text-sm">Initialize Profile</Link>
                  <button className="w-full sm:w-auto px-10 md:px-12 py-4 md:py-5 bg-white/5 border border-white/10 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-white/10 transition-all text-xs md:text-sm">Download Manifesto</button>
               </div>
               <div className="flex items-center justify-center gap-3 pt-6">
                  <ShieldCheck className="text-blue-500" size={16} />
                  <span className="text-[8px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Protocol X-Secure Verified Community</span>
               </div>
            </div>
          </div>
