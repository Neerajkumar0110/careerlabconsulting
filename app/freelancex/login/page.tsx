    ...
    <div className="min-h-screen bg-[#020617] text-white flex flex-col items-center justify-center p-4 sm:p-8 relative overflow-x-hidden selection:bg-blue-500/30">
      ...
      <div className="w-full max-w-[480px] relative z-10 py-12">
        ...
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          >
            Authenticate <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">Node</span>
          </motion.h1>
          <p className="text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-[0.3em] opacity-70">
            Secure Entry into the Elite Ecosystem
          </p>
        </div>
        ...
        >
          ...
          <div className="relative bg-[#050b1d]/80 border border-white/10 rounded-[2.5rem] p-8 sm:p-10 backdrop-blur-2xl shadow-3xl">
            ...
            <form onSubmit={handleLogin} className="space-y-7">
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 ml-1 flex items-center gap-2">
                  <Terminal size={12} className="text-blue-500" /> Network Identity
                </label>
                <div className="relative group">
                  <Mail className="absolute left-5 top-1/3 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-400 transition-colors" size={18} />
                  <input 
                    required
                    type="email" 
                    placeholder="architect@nexus.io"
                    className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-3 mb-4 pl-14 pr-5 text-sm focus:outline-none focus:border-blue-500/50 focus:bg-white/[0.08] transition-all outline-none"
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center px-1">
                  <label className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 flex items-center gap-2">
                    <KeyRound size={12} className="text-indigo-500" /> Security Key
                  </label>
                  <Link href="/forgot-password" className="text-[9px] font-black uppercase text-blue-500 hover:text-white transition-colors">Recovery?</Link>
                </div>
                <div className="relative group">
                  <Lock className="absolute left-5 top-1/3 -translate-y-1/2 text-slate-500 group-focus-within:text-indigo-400 transition-colors" size={18} />
                  <input 
                    required
                    type={showPassword ? "text" : "password"} 
                    placeholder="••••••••••••"
                    className="w-full bg-white/[0.03] border border-white/10 rounded-2xl py-3 mb-4 pl-14 pr-14 text-sm focus:outline-none focus:border-indigo-500/50 focus:bg-white/[0.08] transition-all outline-none tracking-widest"
                    onChange={(e) => setFormData({...formData, password: e.target.value})}
                  />
                  <button 
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-5 top-1/3 -translate-y-1/2 text-slate-500 hover:text-white transition-colors"
                  >
              ...
              <button 
                disabled={isLoading}
                type="submit" 
                className="w-full bg-white text-black py-4 mb-4 rounded-2xl font-black text-xs uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 shadow-xl active:scale-[0.98] disabled:opacity-50 hover:bg-blue-600 hover:text-white group"
              >
                {isLoading ? (
            ...
            <div className="relative my-10 text-center mt-5 mb-10">
        ...
        <div className="text-center mt-10 space-y-8">
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-wider">
            Access denied? <Link href="/freelancex/signup" className="text-white hover:text-blue-400 transition-colors border-b border-blue-500/50 pb-0.5 ml-1">Create Profile</Link>
          </p>
