...
import { Truck, MapPin, ShieldAlert, BarChart3, Zap, Clock, ArrowRight, TrendingDown, Navigation } from 'lucide-react';

function safeParse<T>(raw: string, fallback: T): T {
  try { return JSON.parse(raw) as T; } catch { return fallback; }
}

interface KpiCard     { label: string; value: string; detail: string }
interface CapCard     { title: string; desc: string; icon: string }
interface FeatureItem { icon: string; text: string }

const ICON_MAP: Record<string, React.ElementType> = {
  Clock, ShieldAlert, Navigation, BarChart3, Zap, Truck, MapPin, TrendingDown
};
...
const DEFAULT_CAPS = JSON.stringify([
  { title: 'Predictive Dispatch', desc: 'Algorithms that predict order surges and pre-position vehicles to reduce wait times by up to 35%.', icon: 'Clock' },
  { title: 'Behavioral Safety AI', desc: 'On-edge computer vision to monitor driver fatigue and distraction, providing real-time in-cab coaching.', icon: 'ShieldAlert' },
  { title: 'Dynamic Routing', desc: 'Beyond GPS—AI that adapts to weather, road-wear, and real-time fuel pricing to find the True-Cost path.', icon: 'Navigation' },
]);

const DEFAULT_FEATURES = JSON.stringify([
  { icon: 'BarChart3', text: 'TCO Reduction Engine Active' },
  { icon: 'Zap', text: 'EV Transition Simulation v2.0' },
]);

export default function FleetManagementPage() {
  const { get } = usePageContent('industry-fleet-management');

  const badgeText    = get('hero', 'badge_text',     'Fleet Intelligence & Asset Ops');
  const heroHeadline = get('hero', 'headline_plain', 'ORCHESTRATED');
  const heroAccent   = get('hero', 'headline_accent','LOGISTICS');
  const heroBody     = get('hero', 'body_text',      'Turn your fleet into a self-optimizing network. We deploy AI engines that process millions of data points—from driver behavior to port congestion—to maximize asset utilization and minimize carbon footprints.');
  const heroBtnLabel = get('hero', 'btn_label',      'OPTIMIZE MY FLEET');
  const gradientFrom = get('hero', 'gradient_from',  'from-amber-200');
  const gradientVia  = get('hero', 'gradient_via',   'via-amber-500');
  const gradientTo   = get('hero', 'gradient_to',    'to-orange-700');
  ...
  const detailHeadline = get('detail', 'headline',      'Global\nVisibility');
  const detailBody     = get('detail', 'body_text',     'We provide more than just location tracking. Our Fleet Control Towers integrate telematics with financial data, giving C-suite executives a real-time view of the cost-per-mile across their entire global operation.');
  const featureItems   = safeParse<FeatureItem[]>(get('detail', 'features_json', DEFAULT_FEATURES), []);

  const ctaHeadline   = get('cta', 'headline',  'Master The\nMovement');
  const ctaBody       = get('cta', 'body_text', 'Our fleet architects at DLF Cyber City are ready to build the central brain for your logistics empire.');
  const ctaBtnLabel   = get('cta', 'btn_label', 'START OPERATIONS AUDIT');
  const ctaSubtext    = get('cta', 'subtext',   'Logistics Hub: Gurugram, India');
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#020617] text-white overflow-x-hidden">
      ...
      <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 px-6 text-center">
        <div className="absolute top-0 right-1/4 w-[600px] h-[600px] bg-blue-500/5 rounded-full blur-[120px] -z-10" />
        <div className="max-w-7xl mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 border border-amber-500/20 mb-8 backdrop-blur-md">
            ...
            <span className="text-amber-500 text-xs font-bold uppercase tracking-widest">{badgeText}</span>
          </div>
          ...
          {capabilities.map((item, i) => {
            ...
            return (
              <div key={i} className="p-10 rounded-[3rem] bg-zinc-900/40 border border-white/5 hover:border-amber-500/30 transition-all">
                <div className="mb-6"><Icon className="w-8 h-8 text-amber-500" /></div>
      ...
      <section className="py-24 px-6 relative">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <div>
            <h2 className="text-4xl md:text-7xl font-black mb-8 italic tracking-tighter uppercase leading-tight text-amber-500 whitespace-pre-line">{detailHeadline}</h2>
          ...
          <div className="relative">
            ...
            <div className="relative bg-[#050301] border border-amber-500/20 rounded-[3rem] p-10 shadow-2xl font-mono overflow-hidden">
              ...
              <div className="space-y-6">
                  ...
                  <div className="flex justify-between items-end gap-2 h-20">
                    {[40, 70, 45, 90, 65, 80, 50].map((h, i) => (
                      <div key={i} className="flex-1 bg-amber-500/20 rounded-t-md relative group">
                        <div className="absolute bottom-0 w-full bg-amber-500 rounded-t-md" style={{ height: `${h}%` }} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="p-4 bg-amber-950/20 rounded-xl border border-dashed border-amber-500/30 text-[9px] text-amber-200">
      ...
      <section className="py-32 px-6">
        <div className="max-w-6xl mx-auto bg-gradient-to-br from-amber-950/40 to-black border border-amber-500/20 rounded-[4rem] p-12 md:p-24 text-center backdrop-blur-3xl relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-amber-100/70 text-lg md:text-xl mb-12 max-w-2xl mx-auto">{ctaBody}</p>
            <div className="flex flex-col items-center gap-6">
              <button className="bg-white text-amber-950 px-16 py-6 rounded-2xl font-black text-2xl hover:scale-105 transition-all shadow-2xl">{ctaBtnLabel}</button>
              <div className="text-amber-500 font-mono text-sm tracking-widest uppercase italic">{ctaSubtext}</div>
            </div>
          </div>
        </div>
      </section>
